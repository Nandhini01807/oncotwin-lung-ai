...
BRAIN_CLASSES = [
    {"key": "brain_glioma", "label": "Brain Glioma Tumor Detected", "type": "Malignant", "short": "Glioma"},
    {"key": "brain_menin", "label": "Brain Meningioma Tumor Detected", "type": "Malignant", "short": "Meningioma"},
    {"key": "brain_tumor", "label": "Pituitary Tumor Detected", "type": "Malignant", "short": "Pituitary Tumor"}
]
...
class BrainInferenceService:
    ...
    def _load_weights(self):
        for candidate in ["brain_model.pth", "DenseNet121_Brain.pth"]:
            ...
            if w_path.exists():
                try:
                    ...
                    print(f"[Brain MRI Service] Loaded weights from {w_path}")
                    break
                except Exception as e:
                    print(f"[Brain MRI Service] Error loading weights: {e}")

    def predict(self, file_bytes: bytes, filename: str) -> Dict[str, Any]:
        if not self.weights_loaded:
            self._load_weights()
            if not self.weights_loaded:
                raise RuntimeError("Brain MRI model weights (DenseNet121_Brain.pth) not found.")

        try:
            image_pil = Image.open(io.BytesIO(file_bytes)).convert("RGB")
        except Exception as e:
            raise ValueError(f"Could not decode Brain MRI image: {e}")
        ...
        class_info = BRAIN_CLASSES[pred_idx]
        confidence = round(float(probs[pred_idx]) * 100, 2)

        prob_dist = {
            cls_item["short"]: round(float(probs[i]) * 100, 2)
            for i, cls_item in enumerate(BRAIN_CLASSES)
        }
        ...
        scan_filename = f"scan_brain_{scan_id}.png"
        heatmap_filename = f"heatmap_brain_{scan_id}.png"
        overlay_filename = f"overlay_brain_{scan_id}.png"
        ...
        return {
            ...
            "modelVersion": "DenseNet121_Brain.pth (Kaggle Multi Cancer Dataset)",
            "disclaimer": "This result is generated by DenseNet121 (Brain MRI model) and is intended for clinical decision support under doctor review only. The doctor manually assigns the official clinical stage."
        }

BRAIN_SERVICE = BrainInferenceService()
