...
LUNG_CLASSES = [
    {"key": "lung_bnt", "label": "Benign (Normal Lung Tissue)", "type": "Benign", "short": "Normal/Benign"},
    {"key": "lung_aca", "label": "Lung Adenocarcinoma Detected", "type": "Malignant", "short": "Adenocarcinoma"},
    {"key": "lung_scc", "label": "Lung Squamous Cell Carcinoma Detected", "type": "Malignant", "short": "Squamous Cell Carcinoma"}
]

class LungInferenceService:
    def __init__(self):
    ...
    def _build_model(self) -> nn.Module:
        ...
        model.classifier = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.Linear(256, len(LUNG_CLASSES))
        )
    ...
    def _load_weights(self):
        for candidate in ["lung_model.pth", "DenseNet121_Lung.pth", "densenet121_lung_cancer.pth"]:
            ...
            if w_path.exists():
                try:
                    ...
                    print(f"[Chest X-Ray Service] Loaded weights from {w_path}")
                    break
                except Exception as e:
                    print(f"[Chest X-Ray Service] Error loading weights: {e}")

    def predict(self, file_bytes: bytes, filename: str) -> Dict[str, Any]:
        if not self.weights_loaded:
            self._load_weights()
            if not self.weights_loaded:
                raise RuntimeError("Chest X-Ray model weights (DenseNet121_Lung.pth) not found.")

        try:
            image_pil = Image.open(io.BytesIO(file_bytes)).convert("RGB")
        except Exception as e:
            raise ValueError(f"Could not decode Chest X-Ray image: {e}")
        ...
        class_info = LUNG_CLASSES[pred_idx]
        confidence = round(float(probs[pred_idx]) * 100, 2)

        prob_dist = {
            cls_item["short"]: round(float(probs[i]) * 100, 2)
            for i, cls_item in enumerate(LUNG_CLASSES)
        }
        ...
        scan_filename = f"scan_lung_{scan_id}.png"
        heatmap_filename = f"heatmap_lung_{scan_id}.png"
        overlay_filename = f"overlay_lung_{scan_id}.png"
        ...
        return {
            ...
            "modelVersion": "DenseNet121_Lung.pth (Kaggle Multi Cancer Dataset)",
            "disclaimer": "This result is generated by DenseNet121 (Chest X-Ray model) and is intended for clinical decision support under doctor review only. The doctor manually assigns the official clinical stage."
        }

LUNG_SERVICE = LungInferenceService()
